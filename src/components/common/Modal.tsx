  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-end justify-center p-0 animate-fade-in sm:items-center sm:p-4">
      ...
      <div
        ...
        className={cn(
          "relative z-10 flex max-h-[94dvh] w-full flex-col rounded-t-2xl border border-border bg-surface shadow-popover sm:max-h-[90vh] sm:rounded-xl",
          SIZE_CLASSES[size]
        )}
      >
        ...
        <div className="min-w-0 overflow-y-auto p-4 sm:p-5">{children}</div>
        {footer && <div className="flex flex-wrap items-center justify-stretch gap-2 border-t border-border p-4 sm:justify-end [&>button]:max-sm:flex-1">{footer}</div>}
      </div>
    </div>,
    document.body
  );
