...
export function RmContactBand({ rmId = "rm-sandeep" }: { rmId?: string }) {
  ...
  return (
    <div className="flex min-h-[106px] flex-col justify-center gap-4 rounded-xl border border-border bg-surface p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
      ...
      <div className="flex min-w-0 flex-wrap items-center gap-x-5 gap-y-2 border-t border-border pt-3 sm:shrink-0 sm:border-l sm:border-t-0 sm:py-1 sm:pl-5">
        <a href={`tel:${rm.phone.replace(/\s/g, "")}`} className="flex min-w-0 items-center gap-2 text-sm font-medium text-neutral-700 hover:text-brand-700">
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex min-w-0 items-center gap-2 text-sm font-medium text-status-available hover:underline"
        >
