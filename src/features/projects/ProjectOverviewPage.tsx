    ...
    <div className="flex flex-col pb-3">
      <div className="flex flex-col gap-3 px-4 pt-2 sm:px-6">
        ...
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3 lg:grid-cols-7 lg:[&>*]:h-[104px]">
          ...
          <MetricCard className="h-24" label="Available" value={String(stats.available)} icon={Tag} iconTone="blue" sublabel={`${((stats.available / plots.length) * 100).toFixed(2)}% of total`} progressPercent={(stats.available / plots.length) * 100} onClick={() => navigate("/admin/plot-inventory")} />
          <MetricCard className="h-24" label="Booked" value={String(stats.booked)} icon={CalendarCheck} iconTone="orange" sublabel={`${((stats.booked / plots.length) * 100).toFixed(2)}% of total`} progressPercent={(stats.booked / plots.length) * 100} onClick={() => navigate("/admin/plot-inventory")} />
          <MetricCard className="h-24" label="Sold" value={String(stats.sold)} icon={CheckCircle2} iconTone="green" sublabel={`${((stats.sold / plots.length) * 100).toFixed(2)}% of total`} progressPercent={(stats.sold / plots.length) * 100} onClick={() => navigate("/admin/plot-inventory")} />
